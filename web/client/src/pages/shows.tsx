...
export default function ShowsPage() {
  ...
  const { data, isLoading } = useQuery<any>({
    queryKey: [`/api/search?keyword=series&page=${page}&perPage=24`],
  });

  const allMovies = extractMovies(data).map(normalizeMovie);
  const shows = allMovies.filter(m => m.type === "tv");
  const displayShows = shows.length > 0 ? shows : allMovies;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-screen-2xl mx-auto px-4 md:px-8 pt-24 pb-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-bold" data-testid="text-shows-title">TV Shows</h1>
        </div>
